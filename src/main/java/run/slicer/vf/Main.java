...
public class Main {
    @JSExport
    public static JSPromise<JSMapLike<JSString>> decompileMany(String[] names, Options options) {
        return JSPromise.callAsync(() -> {
            ...
            fernflower.addSource(new ClassSource(names, options.resources(), name0 -> source0(options, name0), outputSink));
            fernflower.addLibrary(new ResourceSource(options.resources(), name0 -> source0(options, name0)));
            fernflower.decompileContext();
            fernflower.clearContext();

            final JSMapLike<JSString> result = JSObjects.create();
            outputSink.output().forEach((k, v) -> result.set(k, JSString.valueOf(v)));
            return result;
        });
    }
    ...
    private static JSPromise<JSString> decompile0(String name, Options options) {
        return JSPromise.callAsync(() -> {
            ...
            fernflower.addSource(new ClassSource(new String[]{name}, options.resources(), name0 -> source0(options, name0), outputSink));
            fernflower.addLibrary(new ResourceSource(options.resources(), name0 -> source0(options, name0)));
            fernflower.decompileContext();
            fernflower.clearContext();

            return JSString.valueOf(outputSink.output().get(name));
        });
    }
